...
-- Generated: 2022-07-27 10:03
-- Model: n1exercici1_model
-- Version: 1.0
...
CREATE TABLE IF NOT EXISTS `n1exercici1`.`glasses` (
  ...
  `glasses_prescription_left` DECIMAL(10,2) NULL DEFAULT NULL,
  `glasses_prescription_right` DECIMAL(10,2) NULL DEFAULT NULL,
  ...
  `glasses_price` DECIMAL(10,2) NULL DEFAULT NULL,
  `sell_id` INT(11) NULL DEFAULT NULL,
  PRIMARY KEY (`glasses_id`),
  INDEX `fk_tag_idx` (`glasses_tag` ASC) VISIBLE,
  INDEX `fk_glasses_buy1_idx` (`sell_id` ASC) VISIBLE,
  CONSTRAINT `fk_tag`
    FOREIGN KEY (`glasses_tag`)
    REFERENCES `n1exercici1`.`tag` (`tag_name`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_glasses_sell`
    FOREIGN KEY (`sell_id`)
    REFERENCES `n1exercici1`.`sell` (`sell_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4;

CREATE TABLE IF NOT EXISTS `n1exercici1`.`sell` (
  `sell_id` INT(11) NOT NULL AUTO_INCREMENT,
  `client_id` INT(11) NOT NULL,
  `seller_name` VARCHAR(255) NULL DEFAULT NULL,
  `sell_date` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`sell_id`),
...
CREATE TABLE IF NOT EXISTS `n1exercici1`.`tag` (
